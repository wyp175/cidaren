...
*/
    ...
    static function checkUpdate(v)
    {
        ...
        var res = sendJSONPOST('cidaren.mcol.cc', '/dl/update.php', json_data.JSONObject);
        res = json_decode(res);
    ...
    static function cidaren(os: Session)
    {
        ...
        var jsonString = os.GetResponseBodyAsString();
        
        if(jsonString!='[]'&&jsonString!='{}' ){
            var responseJSON = json_decode(jsonString);
            if (isJbject(responseJSON) && responseJSON.JSONObject['data'] != null && responseJSON.JSONObject['data']['answer_state']!=null && responseJSON.JSONObject['data']['topic_mode']!=null) {
                ...
                var res = sendJSONPOST('cidaren.mcol.cc', '/q.php', json_data.JSONObject);
                
                res = json_decode(res);
                responseJSON.JSONObject['data']['stem']['remark'] = res.JSONObject['remark'];
                responseJSON.JSONObject['data']['stem']['content'] = res.JSONObject['content']; 


                if(responseJSON.JSONObject['data']['topic_mode'] == 11 || responseJSON.JSONObject['data']['topic_mode'] == 22){
                    var flag = true;
                    var answer = res.JSONObject['answer'];
                    for(var c in responseJSON.JSONObject['data']['options']) {
                        for(var a in answer){
                            if(c['content'].replace(/\s*/g,"") == a[0].replace(/\s*/g,"")){
                                flag = false;
                                c['content'] = ':::::'+c['content']+':::::';
                    ...
                    if(flag){
                        responseJSON.JSONObject['data']['options'][0]['content'] = responseJSON.JSONObject['data']['options'][0]['content']+"_可能的答案:"+res.JSONObject['answer'][0][0]+"";
                    }
                }

                if (responseJSON.JSONObject['data']['topic_mode'] == 31) {
                    // var answer = res.JSONObject['answer'];
                    for(var c in responseJSON.JSONObject['data']['options']) {
                        for(var a in responseJSON.JSONObject['data']['stem']['remark']){
                            if(c['content'] == a['relation']){
                                c['content'] = ':'+c['content']+':';
                            }
                        }
                    }
                ...
                if (responseJSON.JSONObject['data']['topic_mode'] == 32) {
                    var re = responseJSON.JSONObject['data']['stem']['remark'];
                    re = re.replace(/\s+/g, ' ');
                    // log(re);
                    var sp = re.split(" ");
                    var num = 0;
                    var op = responseJSON.JSONObject['data']['options'];
                ...
                if (responseJSON.JSONObject['data']['topic_mode'] == 51) {
                    var re = responseJSON.JSONObject['data']['stem']['remark'];
                    re = re.replace(/.*{(\w+)}.*/g, "$1");
                    // log(re);
                    CDR.setText(re);
                ...
                if (responseJSON.JSONObject['data']['topic_mode'] == 42) {
                    var con = responseJSON.JSONObject['data']['stem']['content'];
                    con = con.replace(/.*{(\w+)}.*/g,"$1");
                    // log(con);
                    for(var c in responseJSON.JSONObject['data']['options']) {
                            if(c['content'].replace(/\s*/g,"") == con.replace(/\s*/g,"")){
                                c['content'] = ':::::'+c['content']+':::::';
                                break;
                            }
                    }
